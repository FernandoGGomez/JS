...
// Funcion que pushea los productos al carrito 
    ...
    eliminarProdCarrito(id){
        ...
        Swal.fire({
            title:"¿Está seguro de retirar el producto del carrito?",
            showDenyButton:true,
            ...
            if(resp.isConfirmed){
                ...
                Swal.fire("Producto retirado del carrito");
    ...
    vaciarCar(){
        ...
        if(carrito != 0){
            ...
            Swal.fire({
                icon: 'info',
                text: 'El carrito actualmente está vacío',
              })
            let detalleCarrito = document.getElementById("idCarrito");
            detalleCarrito.innerHTML = '<p class="carritoVacio">¡Nada por aquí!</p>' ;
    ...
    BotonFiltrar(){
        ...
        marcas.forEach((marca) => {
            contenedor.innerHTML += `<span id='${marca.marca}'>${marca.marca}</span>`;
            document.getElementById(marca.marca).addEventListener('click', filtrarPorMarca())
        });
         ...
         span1.addEventListener("click",()=>filtrarMarca(marcas));
